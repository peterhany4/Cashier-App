      ...
      <ConfirmProvider>
        <CartProvider>
          {!currentUser ? (
            ...
            <div className="h-screen app-bg text-white flex overflow-hidden" dir="rtl">

            {/* Persistent Side Rail — sits on the RIGHT (RTL start) */}
            <aside className="shrink-0 w-60 bg-slate-900/70 backdrop-blur-xl border-l border-slate-800 flex flex-col py-5 shadow-2xl shadow-slate-950/40">
              {/* Brand */}
              <div className="flex items-center gap-3 px-5 mb-6">
                <span className="w-11 h-11 rounded-xl bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 inline-flex items-center justify-center shadow-lg shadow-emerald-950/30">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 font-semibold">{currentUser.username}</p>
                </div>
              </div>

              {/* Nav */}
              <nav className="flex-1 flex flex-col gap-1.5 px-3 mb-2">
                <RailButton active={currentView === 'cashier'} onClick={() => setCurrentView('cashier')} icon={<icons.menu size={19} />} label="شاشة الكاشير" />
                <RailButton active={currentView === 'receipts'} onClick={() => setCurrentView('receipts')} icon={<icons.receipt size={19} />} label="سجل الفواتير" />
                {currentUser.role === 'admin' && (
                  <>
                    <RailButton active={currentView === 'admin'} onClick={() => setCurrentView('admin')} icon={<icons.dashboard size={19} />} label="لوحة التحكم" />
                    <RailButton active={currentView === 'settings'} onClick={() => setCurrentView('settings')} icon={<icons.settings size={19} />} label="الإعدادات" />
                  </>
                )}
              </nav>
              ...
              <div className="px-4 pt-4 border-t border-slate-800 space-y-3">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold ${
                  ...
                  <icons.shield size={14} /> {currentUser.role === 'admin' ? 'مدير النظام' : 'موظف مبيعات'}
                </span>
                <button
