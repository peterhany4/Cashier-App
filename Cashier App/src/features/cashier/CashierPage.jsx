...
export default function CashierPage({ user, onLogout }) {
        ...
        <div className="min-h-screen bg-slate-900 text-white flex flex-col" dir="rtl">
            ...
            <header className="bg-slate-800 border-b border-slate-700 px-6 py-4 flex justify-between items-center shadow-lg">
                <div className="flex items-center gap-4">
                    <h1 className="text-2xl font-black text-emerald-400">نظام المبيعات الذكي</h1>
                    <span className="bg-slate-700 px-3 py-1 rounded text-sm font-semibold">
                        المستخدم: {user.username} ({user.role === 'admin' ? 'مدير' : 'موظف'})
                    </span>
                </div>
                <button
                    onClick={onLogout}
                    className="bg-rose-600 hover:bg-rose-500 text-white font-medium px-4 py-2 rounded-xl transition"
                >
                    تسجيل الخروج
                </button>
            </header>
